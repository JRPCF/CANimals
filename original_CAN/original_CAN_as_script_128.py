...
#             nn.BatchNorm2d(64),
#             nn.ReLU(True),
#             # IGNORE state size. 64 x 128 x 128
...
#               'G Loss:',g_loss.data.cpu().numpy())
    ...
    with open('img_z/img_z_' + str(dt) + '_epoch_' + str(epoch+1) + '_' + str(CROP_SIZE) + '.pkl', 'wb') as f:
        pkl.dump(img_z, f)
    
    #img_list.append(make_grid(img_z, padding=2, normalize=True))
    G.train() # back to training mode  
    
    torch.save(G, 'models/G_' + str(dt) + '_epoch_' + str(epoch+1) + '_' + str(CROP_SIZE) + '.pt')
    torch.save(D, 'models/D_' + str(dt) + '_epoch_' + str(epoch+1) + '_' + str(CROP_SIZE) + '.pt')
